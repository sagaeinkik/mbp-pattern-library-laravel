...
export default function UploadPlugin(breadcrumbs: BreadcrumbItem[]) {
    // Form data
        ...
        <AppLayout breadcrumbs={allBreadcrumbs}>
            <div className="text-bread max-w-8xl p-4">
                ...
                <form className="my-4" onSubmit={handleSubmit}>
                    ...
                    <Input type="file" name="pluginFile" id="pluginFile" className="mt-2 mb-4" onChange={handleFileChange} accept=".zip" ref={fileInputRef} />
                    <Button disabled={processing}>{processing ? "Submitting" : "Submit"}</Button>
                { successMessage && <><p className="my-4">{ successMessage }</p><a href={route("wordpress.all")} className="text-primary-foreground-links hover:text-foreground underline decoration-dotted">Return</a></>}
                </form>
            </div>
        </AppLayout>
    )
