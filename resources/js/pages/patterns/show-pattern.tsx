...
export default function ShowPattern({ pattern }: { pattern: Pattern }) {
    return (
        <PatternsLayout title={pattern.title + " details"}>
            <Link href={route("patterns.all")} className="mb-4 flex gap-1 text-sm text-foreground/60 hover:text-foreground items-center w-fit"><ArrowLeft size={15} />Back to all categories</Link>
            <div className="flex justify-between items-center w-full">
                <h1 className="text-2xl">{pattern.title}</h1>
                <Link as="button" href="#" className="my-2 bg-secondary py-2 px-4 rounded-md text-sm hover:bg-primary">Handle pattern</Link>
            </div>
            {/* Pattern description */}
