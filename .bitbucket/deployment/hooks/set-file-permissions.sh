...
#
# To learn more, visit: https://deploy-laravel.com/laravel-file-permissions
...
on_exit() {
    ...
    if [[ "$script_status_code" -ne 0 ]]; then
        echo "For more information about file permissions, please visit: https://deploy-laravel.com/laravel-file-permissions"
    fi
...
if [[ -z "$http_user" ]]; then
    echo "Could not detect which user serves web requests. You must manually set the \"\$http_user\" variable in the \"set-file-permissions\" hook."
...
if [[ -z "$(command -v "setfacl")" ]]; then
    echo "We need \"setfacl\" to properly set file permissions, but it is not installed. You can install it using \"sudo apt install acl\"."

    exit 1
fi

#http_user_group=$(id -Gn "$http_user")
http_user_group="www-data"
if [[ $(wc -w <<< "$http_user_group") != "1" ]]; then
    echo "Can not set file permissions. The \"$http_user\" user belongs to multiple groups, we can't decide which group we should use. You must manually set the \"\$http_user_group\" variable in the \"set-file-permissions\" hook. User \"$http_user\" belongs to these groups: $http_user_group"

    exit 1
fi

if ! id -nG "$deployment_user" | grep -q "\b$http_user_group\b"; then
    echo "Can not set file permissions. The \"$deployment_user\" user should be in the \"$http_user_group\" group. You can do this using \"sudo usermod -aG $http_user_group $deployment_user\"."
...
if [[ "$filesystems_config_permission_1" != "775" ]] || [[ "$filesystems_config_permission_2" != "775" ]]; then
    echo "You must set the default directory permissions to 0775 in your \"filesystems.php\" config file. This must be done for both the \"local\" disk and the \"public\" disk. With the current setting you will get file permission errors. Make sure you set them to 0775, not 775."
