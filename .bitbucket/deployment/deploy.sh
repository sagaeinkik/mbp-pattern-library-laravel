...
if [[ ! -x "$(command -v "$php_executable")" ]]; then
    echo "The PHP executable is set to \"$php_executable\", but that file either does not exist, or is not executable."
...
if [[ ! -s "$real_env_file_path" ]]; then
    ...
    echo "Your \"$real_env_file_path\" file is empty. Run the deployment again after you've filled it in."
...
if ! [[ $("$php_executable" artisan tinker --help) =~ "--execute" ]]; then
    echo "Laravel Tinker is not installed, or you are using an outdated version. Laravel Tinker version ^2.0 is required."
...
# Keep only the 2 newest release directories.
